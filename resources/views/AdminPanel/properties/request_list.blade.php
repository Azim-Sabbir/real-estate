...
@section('main-section')
    <div class="container">
        <div class="container-fluid">
            <div class="mt-4 ">
                <h2>User Requested Properties</h2>
            ...
            <div class="mt-4">
                <table class="table table-hover table-striped" id="data">
                    ...
                    @foreach ($pro as $item)
                        <tr>
                            <th scope="row"
                                @if ($item->public == 1) class="text-success">Public @else
                            ...
                            <th scope="row"
                                @if ($item->featured == 1) class="text-success">Active @else
                                    class="text-danger"> Disabled
                                @endif</th>
                            <th scope="row">{{ ucfirst($item->purpose) }}</th>
                            <th scope="row">{{ $item->Cate->name }}</th>
                            <th scope="row">
                                <img height="30rem" class="rounded" style="cursor: pointer" data-fancybox="gallery" data-src="{{ asset('/storage/property/' . $item->image) }}" src="{{ asset('/storage/property/' . $item->image) }}" alt="Error">
                            </th>
                            ...
                            <th scope="row">{{ $item->owner ? $item->owner->name : "..." }}</th>
                            <th scope="row">{{ $item->status }}</th>

                            <th scope="row">
                                <a class="btn btn-success btn-sm" href="{{ route('property_edit', $item->id) }}">
                                    <i class="fa fa-edit" aria-hidden="true"></i>
                                </a>
                                @if (session()->get('AdminUser')['type'] == 'R')
                                    <a class="btn btn-danger btn-sm" onclick="return confirm('Sure to delete?')" href="{{ route('property_request_delete', $item->id) }}">
